...
<div style="overflow-x: auto;">
    ...
    foreach ($information as $info) {
     ?>
<tr>
    <td><?php echo $info['Stuid']; ?></td>
    <td><?php echo $info['sname']; ?></td>
    <td><?php echo $info['branch']; ?></td>
    <td><?php echo $info['liveBack']; ?></td>
    <td style="white-space:nowrap;">
        <button type="submit"  class="btn btn-primary" id="approve" name="approve" value="<?php echo $info['Stuid'] ; ?>">Approved</button>
        <button type="submit" class="btn btn-primary"  id="cancle" name="cancle"  value="<?php echo $info['Stuid'] ; ?>" >Cancle</button>
</td>
...
?>
    ...
    if(isset($_POST['approve'])){
        $eno = $_POST['approve'];
        $query4 = "select * from `sturegister` where `enroll` = '$eno'";
        $data = mysqli_query($conn,$query4);
        $result4 = mysqli_fetch_assoc($data);
        $sname = $result4['name'];

        $query3 = "INSERT INTO `approval`( `eno`, `sname`, `status`,`AppDate`,`companyid`) VALUES ('$eno','$sname','yes','$AppDate','$cid')";
    ...
    if(isset($_POST['cancle'])){
        ...
        $result4 = mysqli_fetch_assoc($data);
        $sname = $result4['name'];
        $query5 = "INSERT INTO `approval`( `eno`, `sname`, `status`,`AppDate`,`companyid`) VALUES ('$eno','$sname','no','$AppDate','$cid')";
        $result5 = mysqli_query($conn ,$query5);
        echo "<script>alert('Cancle Successfully');</script>";
    }
